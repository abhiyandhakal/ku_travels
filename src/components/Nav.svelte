<script lang="ts">
	...
	if (typeof window !== "undefined") {
		active =
			(navItems.find(({ href }) => href === window.location.pathname)
				?.id as number) ?? 0;
	}
...
<nav class={hamburgerClicked ? "show-nav" : ""}>
	<div>
		...
		>
			...
			<span
				class={`hamburger ${hamburgerClicked ? "active" : ""}`}
		...
		<ul class={hamburgerClicked ? "show" : ""} role="navigation">
			{#each navItems as { id, href, text, target } (id)}
				<li class="nav-item">
					<span class="link-container">
						<a
							class={`hover-animate ${id === active ? "active" : null}`}
							{href}
							{target}
						>
							{text}
